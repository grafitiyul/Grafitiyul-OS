...
if (mdOut) {
  ...
  L.push('');
  L.push('## Safest correction path');
  L.push('');
  L.push('GOS never writes to Airtable, so every option below is a manual source-side action.');
  L.push('');
  L.push('**Recommended: change nothing in Airtable, and do not pass `--accept-rejected-dates`.**');
  L.push('');
  L.push('The gate is currently refusing to plan 45 records that contain, between them, one customer registration whose Pipedrive deal no longer exists. Nothing operational is behind the gate. The two ways forward:');
  L.push('');
  L.push('| option | effect | risk |');
  L.push('| --- | --- | --- |');
  L.push('| **A. Review the single flagged record, then re-run the plan** | Decide `recSX9jmU0r1EnhuH` on its merits; the other 44 need no action | none — read-only until the cutover is separately approved |');
