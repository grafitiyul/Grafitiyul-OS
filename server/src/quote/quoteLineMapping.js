...
export function toClientLine(l) {
  return {
    ...
    // Per-line discount INTENT (the resolved money is a sibling line_discount
    // row, regenerated by compose — see builderCompose.js).
    discountPercent: l.discountPercent != null ? l.discountPercent : null,
    discountFixedMinor: l.discountFixedMinor != null ? Number(l.discountFixedMinor) : null,
...
export function lineToData(ln, i) {
  ...
  return {
    ...
    // Discount intent: at most one positive value survives (percent wins).
    discountPercent:
      Number.isFinite(Number(ln.discountPercent)) && Number(ln.discountPercent) > 0 && Number(ln.discountPercent) <= 100
        ? Number(ln.discountPercent)
        : null,
    discountFixedMinor:
      !(Number.isFinite(Number(ln.discountPercent)) && Number(ln.discountPercent) > 0) &&
      Number.isFinite(Number(ln.discountFixedMinor)) &&
      Number(ln.discountFixedMinor) > 0
        ? BigInt(Math.round(Number(ln.discountFixedMinor)))
        : null,
    vatMode,
