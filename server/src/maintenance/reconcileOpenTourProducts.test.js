...
test('idempotent: a done marker is skipped (no re-run)', async () => {
  const c = fakeClient({ job: { key: 'reconcile_open_tour_products_v1', status: 'done', startedAt: new Date(), attempts: 1 } });
  const res = await runReconcileOpenTourProductsOnce(c, null);
...
test('concurrency-safe: a freshly-running marker (another instance) is skipped', async () => {
  const c = fakeClient({ job: { key: 'reconcile_open_tour_products_v1', status: 'running', startedAt: new Date(), attempts: 1 } });
  const res = await runReconcileOpenTourProductsOnce(c, null);
  assert.equal(res.skipped, true);
});

test('retry-safe: a failed marker is reclaimed and completed on the next run', async () => {
  const c = fakeClient({ job: { key: 'reconcile_open_tour_products_v1', status: 'failed', startedAt: new Date(0), attempts: 1 } });
  const res = await runReconcileOpenTourProductsOnce(c, null);
  assert.equal(res.done, true);
  assert.equal(c.state.job.status, 'done');
...
test('retry-safe: a STALE running marker (crashed instance) is reclaimed', async () => {
  ...
  const c = fakeClient({ job: { key: 'reconcile_open_tour_products_v1', status: 'running', startedAt: old, attempts: 1 } });
  const res = await runReconcileOpenTourProductsOnce(c, null);
  assert.equal(res.done, true);
});
