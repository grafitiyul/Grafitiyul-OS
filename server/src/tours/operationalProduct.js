...
// Re-align a tour's DEAL registrations to their deal's actual sellable variant.
// The invariant (since Part B) is registration.productVariantId === the deal's
// chosen variant; syncDealRegistration enforces it going forward, but rows
// created BEFORE that (deal-WON backfill / pre-fix era) snapshotted the tour's
// THEN-current variant — so a plain "Graffiti Tour" deal can carry a stale
// WORKSHOP variant on its registration, which then makes derivation re-derive
// workshop. Realigning to the deal (plain, or null → plain base) is what lets a
// plain-only tour finally resolve to plain. Returns the count changed.
export async function realignDealRegistrationVariants(client, tourEventId) {
  const regs = await client.ticketRegistration.findMany({
    ...
    select: { id: true, productVariantId: true, deal: { select: { productVariantId: true } } },
  });
  let changed = 0;
  for (const r of regs) {
    const want = r.deal?.productVariantId ?? null;
    if ((r.productVariantId ?? null) !== want) {
      await client.ticketRegistration.update({ where: { id: r.id }, data: { productVariantId: want } });
      changed += 1;
    }
  }
  return changed;
...
export async function reconcileAllOpenTourProducts(
  ...
  for (;;) {
    ...
    for (const t of batch) {
      ...
      let res;
      try {
        // FIRST re-align stale deal-registration variants (the real root cause of
