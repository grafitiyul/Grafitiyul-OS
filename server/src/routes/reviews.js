...
// Approve the current checkpoint and let the learner continue.
router.post('/attempts/:id/approve', async (req, res) => {
...
// Return the current segment for correction. Learner goes back to the start
// of the segment (after the previous checkpoint, or flow start).
router.post('/attempts/:id/return', async (req, res) => {
  const { note } = req.body;
  const attempt = await prisma.attempt.findUnique({
    where: { id: req.params.id },
    include: { flow: { include: { nodes: true } } },
