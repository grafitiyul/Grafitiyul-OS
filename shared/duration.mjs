// Duration display — THE shared formatter. durationHours (numeric) is the only
// source of truth; the localized text is derived here, never stored. Used by
// the client (variant/product editors) AND the server (confirmation-email
// composer), so an email and the admin UI can never disagree on wording.
//
//   1   → שעה / 1 hour
