// Official WhatsApp glyph (green roundel + white handset), shared wherever the
// product surfaces WhatsApp — settings card, connections page, chat tab.
// The path data is exported so the module favicon (shell/moduleFavicon.js) can
// serialize the exact same mark — ONE source for the brand geometry.
export const WHATSAPP_LOGO_VIEWBOX = '0 0 24 24';
export const WHATSAPP_LOGO_PATHS = [
  {
    fill: '#25D366',
    d: 'M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.82 11.82 0 0 1 8.413 3.488 11.82 11.82 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24z',
  },
  {
    fill: '#FFF',
    d: 'M12.05 21.785h-.005a9.87 9.87 0 0 1-5.03-1.378l-.36-.214-3.742.982.999-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.002-5.45 4.436-9.884 9.889-9.884a9.82 9.82 0 0 1 6.991 2.898 9.82 9.82 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.884 9.884m5.422-7.403c-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.149-.173.198-.297.297-.495.099-.198.05-.372-.025-.521-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479s1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347',
  },
];

export default function WhatsAppLogo({ size = 16, className }) {
  return (
    <svg viewBox={WHATSAPP_LOGO_VIEWBOX} width={size} height={size} aria-hidden className={className}>
      {WHATSAPP_LOGO_PATHS.map((p, i) => (
        <path key={i} fill={p.fill} d={p.d} />
      ))}
    </svg>
  );
}
