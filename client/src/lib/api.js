...
export const api = {
  contentItems: {
    ...
    update: (id, data) => request(`/api/items/content/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  ...
  questionItems: {
    ...
    update: (id, data) => request(`/api/items/questions/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  ...
  flows: {
    ...
    saveNodes: (id, nodes) => request(`/api/flows/${id}/nodes`, {
      method: 'PUT',
      body: JSON.stringify({ nodes }),
    }),
  },
  attempts: {
    create: (flowId, learnerName) => request('/api/attempts', {
      method: 'POST',
      body: JSON.stringify({ flowId, learnerName }),
    }),
    get: (id) => request(`/api/attempts/${id}`),
    answer: (id, payload) => request(`/api/attempts/${id}/answer`, {
      method: 'POST',
  ...
  reviews: {
    ...
    returnForFix: (id, note) => request(`/api/reviews/attempts/${id}/return`, {
      method: 'POST',
